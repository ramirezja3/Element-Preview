...
import type { ElementUsageItemModel } from '../repository/element-usage-preview.repository.js';
import {
	ELEMENT_USAGE_PREVIEW_BATCH_SIZE,
	ELEMENT_USAGE_PREVIEW_QUERY_PARAM,
	ELEMENT_USAGE_PREVIEW_SESSION_PARAM,
} from '../constants.js';

interface StoredUsageSession {
	items: Array<ElementUsageItemModel>;
	hasMore: boolean;
	currentIndex: number;
}

function storageKeyFor(elementKey: string, sessionId: string): string {
	return `elementUsagePreview:${elementKey}:${sessionId}`;
}

@customElement('element-usage-preview-nav')
export class ElementUsagePreviewNavElement extends UmbLitElement {
	...
	@state()
	private _isLoadingPopoverMore = false;
	...
	@state()
	private _popoverTick = 0;

	#elementKey?: string;
	#sessionId?: string;
	#storageKey?: string;
	#items: Array<ElementUsageItemModel> = [];
	#iterator?: AsyncGenerator<ElementUsageItemModel, void, void>;
	...
	async #init(): Promise<void> {
		...
		if (!elementKey || !sessionId) return;
	...
	async #peekOneMore(): Promise<void> {
		const { value, done } = await this.#iterator!.next();
		if (done) {
			this._hasMore = false;
		} else {
	...
	async #ensureIterator(): Promise<void> {
	...
	#nextPreviewableIndex(from: number): number | null {
		for (let i = from; i < this.#items.length; i++) {
			if (this.#items[i].isPreviewable) return i;
		}
		return null;
	}

	#prevPreviewableIndex(from: number): number | null {
	...
	async #goNext(): Promise<void> {
		...
		try {
			...
			if (idx !== null) await this.#navigateTo(idx);
		} finally {
	...
	async #jumpTo(index: number): Promise<void> {
		if (this._isNavigating || index === this._currentIndex) return;
		this._isNavigating = true;
		try {
	...
	async #loadMoreForPopover(): Promise<void> {
		if (this._isLoadingPopoverMore) return;
	...
	async #notifyNavigationFailed(): Promise<void> {
	...
	#previewablePosition(): number {
		let count = 0;
		for (let i = 0; i <= this._currentIndex; i++) {
			if (this.#items[i]?.isPreviewable) count++;
		}
		return count;
	}

	#previewableLoadedCount(): number {
	...
	#previewableEntries(): Array<{ item: ElementUsageItemModel; index: number }> {
		return this.#items.reduce<Array<{ item: ElementUsageItemModel; index: number }>>((acc, item, index) => {
			if (item.isPreviewable) acc.push({ item, index });
			return acc;
		}, []);
	}

	#readStorage(): StoredUsageSession | null {
		try {
			const raw = sessionStorage.getItem(this.#storageKey!);
			return raw ? (JSON.parse(raw) as StoredUsageSession) : null;
		} catch (error) {
	...
	#writeStorage(): void {
		...
		const payload: StoredUsageSession = {
			items: this.#items,
	...
	override render() {
		if (!this._active || this._currentIndex < 0) return nothing;
		...
		const nextDisabled =
			this._isNavigating || (!this._hasMore && this.#nextPreviewableIndex(this._currentIndex + 1) === null);
...
declare global {
	interface HTMLElementTagNameMap {
		'element-usage-preview-nav': ElementUsagePreviewNavElement;
	}
}
